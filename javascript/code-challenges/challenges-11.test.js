...
const count = (target, input) => {
  ...
  input
    .map(element => element
      .forEach(element => element === target ? ct++:false));
  return ct;
  // Solution code here...
};
...
const totalSum = (input) => {
  ...
  let marco = input
    ...
    .split('')
    .map(element => element!==','? array.push(element):false);
  let polo = array
...
let findMaleAndFemale = (data) => {
  let marco = data
    .filter(element => element.gender === 'female' || element.gender === 'male' )
    .map(element => element.name);
  let traveler = marco.reduce((acc,curr) => {
    acc = `${acc} and ${curr}`;
    return acc;
  },``)
    .slice(5);
...
let findShortest = (data) => {
  let marco = data.sort((a,b) => a.height - b.height);
  let polo = marco.slice(0,1);
  let traveler = polo[0].name;
