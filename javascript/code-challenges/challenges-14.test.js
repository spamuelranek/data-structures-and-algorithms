...
const toTitleCase = (arr) => {
  let marco = arr.map(element => element.split(''));

  let polo = marco.map(element => {
    let travler = element[0].toUpperCase();
    for(let i=1; i<element.length; i++){
      travler = travler + element[i];
    }
    return travler;
  });
...
let biggerThanLuke = (arr) => {
  let array =[];
  arr.map(element => element.mass > 77 ? array.push(element.name): false);
  let polo = array.join(' - ');
  return polo;
  // Solution code here...
};
...
const sortBy = (property, arr) => {
  if(property === 'name'){
    console.log('inside');
    let marco = arr.sort((a,b) => b.name>a.name ? -1: 1);
    console.log(marco);
    return arr.sort();
  } else {
    let polo = arr.sort((a,b) => a[property]-b[property]);
    console.log(polo);
...
const detectTicTacToeWin = (board) => {
  ...
  if((board[0][0] === board[1][0]) && (board[1][0]=== board[2][0]) && (board[2][0]=== 'X')){
    return true;
  } else if((board[0][1] === board[1][1]) && (board[1][1] === board[2][1]) && (board[2][1] === ('X' || 'O'))){
    return true;
  } else if((board[0][2] === board[1][2]) && (board[1][2] === board[2][2]) && (board[2][2]=== ('X' || 'O'))){
    return true;
  } else if((board[0][0] === board[0][1]) && (board[0][1] === board[0][2]) && (board[0][2] === ('X' || 'O'))){
    return true;
  } else if((board[1][0] === board[1][1]) && (board[1][1] === board[1][2]) && (board[1][2] === ('X' || 'O'))){
    return true;
  } else if((board[2][0] === board[2][1]) && (board[2][1] === board[2][2]) && (board[2][2] === ('X' || 'O'))){
    return true;
  } else if((board[0][0] === board[1][1]) && (board[1][1] === board[2][2]) && (board[2][2] === ('O'))){
    return true;
  } else if((board[0][2] === board[1][1]) && (board[1][1] === board[2][0]) && (board[2][0] === ('X' || 'O'))){
    return true;
  } else{
    return false;
  }
