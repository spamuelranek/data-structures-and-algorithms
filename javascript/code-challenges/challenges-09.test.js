...
const updateNumbers = (obj) => Object.entries(obj).map(element => `${element[0]}: ${element[1]}`);
...
const getHouses = (arr) => {
  let houses = [];
  let marco = arr.map(element => element.house);
  // Solution code here...
  return marco;
};
...
const hasChildrenValues = (arr, character) => {
  let bool = false;
  let child = arr.map(element => {
    (element.name === character) && Object.values(element).length > 3? bool = true: false;
  });
  return bool;
  // Solution code here...
