...
// i want to find the length of each string in the array
// which ever has the highest value return that index
// Solution code here...
  let marco = arr.map(element => element.length);
  let polo = marco.reduce((acc,curr) => {
    acc = acc > curr ? acc:curr;
    return acc;
  },0);
  let index = 0;
  marco.map((element,idx) => {
    if(element === polo){
      index = idx;
    }
  } );
...
const firstLetters = (arr) => {
  let marco = arr.map(element => element.slice(0,1));
  return marco;
  // Solution code here...
};
...
const findHappiness = (arr) => {
  let array =[];
  arr.map(element => {
    element.includes(':)') ? array.push(element):false;
  });
  return array;
  // Solution code here...
};
...
const standardizePhoneNumbers = (arr) => {
  let regex = /[() -]/g;
  let marco = arr.map(element => element.replace(regex,''));
  return marco;
  // Solution code here...
};
...
const onlyOddChars = (str) => {
  let marco = str.split('');
  let array =[];
  let polo = marco.map((element,idx)=> {
    if(idx % 2 > 0){
      array.push(element);
    }
  });
  let traveler = array.reduce((acc,curr) => {
    acc = acc+curr;
    return acc;
  },'');
...
const allHappy = (arr) => {
  let marco = findHappiness(arr);
  if(marco.length === arr.length){
    return true;
  } else {
    return false;
  }
